function _parseMillisecondsIntoReadableTime(timestamp){
    ...
    const hours = '0' +  date.getHours(); // uren krijgen
    const minutes = '0' + date.getMinutes(); ; // minuten krijgen
...
let placeSunAndStartMoving = (totalMinutes, sunrise) => {
    ...
    minutesLeft.innerHTML = `${totalMinutes - minutesSunUp} minutes`;
...
let getAPI = async (lat, lon) => {
    // Endpoint met x en y coordinaten als dynamic URL
    const ENDPOINT = `http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=3f94f90f0a2e8a001cf72f37c1707325&units=metric&lang=nl&cnt=1`;
